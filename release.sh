...
WHEEL=$(ls lather*.whl)
curl -H "Authorization: token ${GITHUB_TOKEN}" -H "Accept: application/vnd.github.v3+json" -H "Content-type: application/octet-stream" --data-binary @${WHEEL} "https://uploads.github.com/repos/saethlin/rust-lather/releases/${ID}/assets?name=${WHEEL}"
