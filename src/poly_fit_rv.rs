...
pub fn fit_rv(rv: &[f64], ccf: &[f64]) -> f64 {
    ...
    let rv_matrix_values: Vec<f64> = std::iter::repeat(1.0).take(7).chain(peak_rv.clone()).chain(peak_rv.map(|x| x.powi(2))).collect();
    
    let rv_matrix = Matrix::new(3, 7, rv_matrix_values).transpose();
    let ccf_matrix = Matrix::new(7, 1, peak_ccf);
    let coefficients = (rv_matrix.transpose() * rv_matrix.clone()).inverse().unwrap() * rv_matrix.transpose() * ccf_matrix;
