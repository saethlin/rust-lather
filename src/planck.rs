...
fn planck(wavelength: f64, p: &mut f64) -> f64 {
    ...
    let k_b = 1.380e-23;
    2.0*h*c*c*1.0/(wavelength*wavelength*wavelength*wavelength*wavelength*(exp_m1((h*c)/(wavelength*k_b* (*p)))))
}
...
pub fn planck_integral(temperature: f64, wave_min: f64, wave_max: f64) -> f64 {
    ...
    w.qags(planck, &mut t, wave_min, wave_max, 0f64, 1e-7f64, 1000, &mut result, &mut error);
